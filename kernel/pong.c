...
void pong_process(PROCESS self, PARAM param) {
  int window_id = wm_create(15, 10, PONG_WINDOW_WIDTH, PONG_WINDOW_HEIGHT);
  char* buffer = wm_get_buffer(window_id);
  int state = PONG_STATE_INIT;
  int x, y, dx, dy, racket = 0;
  ...
  while(1) {
    ...
    case PONG_STATE_MOVE:
      // Move ball
      if (y == 0) dy = 1;
      if (y == PONG_WINDOW_HEIGHT - 1) dy = -1;
      if (x == PONG_WINDOW_WIDTH - 1) dx = -1;
      if (x == 0) {
      ...
      case 'q':
        ...
        break;
      case 'a':
        if (racket + PONG_RACKET_HEIGHT != PONG_WINDOW_HEIGHT) racket++;
        break;
      }
