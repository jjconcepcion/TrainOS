...
void draw_window_frame(WM* window, BOOL is_top) {
  ...
  poke_screen_buffer(frame_x, frame_y,
         is_top ? FRAME_FOCUS_TOP_LEFT : FRAME_NO_FOCUS_TOP_LEFT);
  poke_screen_buffer(frame_x + frame_width, frame_y,
         is_top ? FRAME_FOCUS_TOP_RIGHT : FRAME_NO_FOCUS_TOP_RIGHT);
  poke_screen_buffer(frame_x, frame_y + frame_height,
         is_top ? FRAME_FOCUS_BOTTOM_LEFT : FRAME_NO_FOCUS_BOTTOM_LEFT);
  poke_screen_buffer(frame_x + frame_width, frame_y + frame_height,
         is_top ? FRAME_FOCUS_BOTTOM_RIGHT : FRAME_NO_FOCUS_BOTTOM_RIGHT);
  char ch = is_top ? FRAME_FOCUS_HORIZONTAL : FRAME_NO_FOCUS_HORIZONTAL;
...
void init_wm() {
  wm_port = create_process(process_window_manager, 6, 0, "Window Manager");
}
